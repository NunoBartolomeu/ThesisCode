package com.ledger.app.services.key_management.implementations

import com.ledger.app.services.key_management.KeyManagementService
...
@Service
class KeyManagementServiceLocal(
    @Value("\${system.key.path:./system_keys/system_private_key.pem}") private val keyPathStr: String
    ...
    override fun getSystemKeyPair(): KeyPair = keyPair
    ...
    private fun initializeKeyPairAndCert(): Pair<KeyPair, X509Certificate> {
        ...
        val newCert = createSystemCertificate(newKeyPair)
    ...
    private fun testKeyAndCertificate(certificate: X509Certificate, keyPair: KeyPair): Boolean {
        try {
        ...
        val test = "test"
        val signature = SignatureProvider.sign(test, keyPair.private, null)
        if (!SignatureProvider.verify(test, signature, certificate.publicKey, null)) {
